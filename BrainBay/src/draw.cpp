...
{
    HDC hdc;
	// LabVIEW-style color scheme
	// pen_white  : highlight/selection border (white)
	// pen_blue   : module body border (medium gray)
	// pen_ltblue : light accent pen
	// pen_red    : connection wire (LabVIEW orange wire)
	// brush_blue : title bar fill (LabVIEW deep blue)
	// brush_white: unused / keep white
	// brush_orange: output port (LabVIEW orange)
	// brush_ltorange: selection halo
	// brush_yellow: input port (LabVIEW green)
	// brush_ltgreen: module body fill (dark gray)
	DRAW.pen_white    = CreatePen(PS_SOLID, 1, PALETTERGB(220, 220, 220));  // light gray highlight
	DRAW.pen_blue     = CreatePen(PS_SOLID, 1, PALETTERGB(90,  90,  95));   // module border gray
	DRAW.pen_ltblue   = CreatePen(PS_SOLID, 1, PALETTERGB(130, 130, 140));  // lighter border
	DRAW.pen_red      = CreatePen(PS_SOLID, 2, PALETTERGB(255, 165,  0));   // LabVIEW orange wire (2px)
	DRAW.brush_blue   = CreateSolidBrush(PALETTERGB(30,  87, 149));         // LabVIEW title bar blue
	DRAW.brush_white  = CreateSolidBrush(PALETTERGB(255, 255, 255));
	DRAW.brush_orange = CreateSolidBrush(PALETTERGB(255, 140,  0));         // output port orange
	DRAW.brush_ltorange = CreateSolidBrush(PALETTERGB(255, 255, 255));      // selection halo white
	DRAW.brush_yellow = CreateSolidBrush(PALETTERGB(0,   200, 100));        // input port green
	DRAW.brush_ltgreen = CreateSolidBrush(PALETTERGB(62,  62,  66));        // module body dark gray
	DRAW.particles=0;
...
void draw_object(HDC hdc, WORD t)
{
	...
	// --- Selection halo (white outer glow) ---
	if (objects[t] == actobject)
	{
		HPEN hPenSel = CreatePen(PS_SOLID, 2, PALETTERGB(255, 255, 255));
		HPEN hOld = (HPEN)SelectObject(hdc, hPenSel);
		SelectObject(hdc, GetStockObject(NULL_BRUSH));
		RoundRect(hdc, x-5, y-5, x+w+5, y+h+5, 12, 12);
		SelectObject(hdc, hOld);
		DeleteObject(hPenSel);
	}

	// --- Module body (dark gray, rounded) ---
	SelectObject(hdc, DRAW.pen_blue);
	SelectObject(hdc, DRAW.brush_ltgreen);
	RoundRect(hdc, x, y, x+w, y+h, 8, 8);

	// --- Title bar (LabVIEW blue, rounded top) ---
	SelectObject(hdc, DRAW.brush_blue);
	SelectObject(hdc, DRAW.pen_blue);
	RoundRect(hdc, x, y, x+w, y+16, 8, 8);
	// Fill bottom half of title bar corners (make it look like flat bottom)
	RECT rcTitleFill = { x+1, y+8, x+w-1, y+16 };
	FillRect(hdc, &rcTitleFill, DRAW.brush_blue);

	// --- 3D bevel: top-left highlight ---
	HPEN hPenHi = CreatePen(PS_SOLID, 1, PALETTERGB(110, 110, 115));
	HPEN hPenOld = (HPEN)SelectObject(hdc, hPenHi);
	MoveToEx(hdc, x+1, y+h-2, NULL); LineTo(hdc, x+1, y+1); LineTo(hdc, x+w-2, y+1);
	DeleteObject(SelectObject(hdc, hPenOld));

	// --- 3D bevel: bottom-right shadow ---
	HPEN hPenSh = CreatePen(PS_SOLID, 1, PALETTERGB(25, 25, 27));
	SelectObject(hdc, hPenSh);
	MoveToEx(hdc, x+1, y+h-2, NULL); LineTo(hdc, x+w-2, y+h-2); LineTo(hdc, x+w-2, y+1);
...
void draw_captions(HDC hdc, WORD t)
{
	...
	char szdata[100];
		...
		SetBkColor (hdc, PALETTERGB(30, 87, 149));        // LabVIEW title bar blue
		SetTextColor (hdc, PALETTERGB(255, 255, 255));     // white text
...
//		SetTextColor (hdc, PALETTERGB(100,255,200));
		SetBkColor(hdc, PALETTERGB(45, 45, 48));           // canvas dark bg
		SetTextColor(hdc, PALETTERGB(200, 200, 200));      // light gray port labels
