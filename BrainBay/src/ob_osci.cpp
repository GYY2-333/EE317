...
OSCIOBJ::OSCIOBJ(int num) : BASE_CL()	
	  {
		...
		bkcol=RGB(30, 30, 32);                     // LabVIEW dark oscilloscope background
		bkbrush=CreateSolidBrush(bkcol);

		gridcol=RGB(70, 90, 110);                  // muted blue-gray grid
		gridpen=CreatePen(PS_SOLID,1,gridcol);

		captcol=RGB(160, 160, 170);                // light gray caption text
		captpen=CreatePen(PS_SOLID,1,captcol);
		redraw=TRUE;		
		showgroupsignal=0xffffffff;

		// LabVIEW-style multi-channel signal colors
		static const COLORREF lv_sigcols[8] = {
			RGB(255, 165,   0),   // orange
			RGB(  0, 200, 100),   // green
			RGB( 80, 180, 255),   // light blue
			RGB(255,  80,  80),   // red
			RGB(200, 130, 255),   // purple
			RGB(255, 230,   0),   // yellow
			RGB(  0, 220, 220),   // cyan
			RGB(255, 140, 100),   // salmon
		};
		for (t=0;t<MAX_EEG_CHANNELS;t++) 
		{
			sigcol[t] = lv_sigcols[t % 8]; sigsize[t]=1;
